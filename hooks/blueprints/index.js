...
module.exports = function(sails) {	
  ...
  function _bindShadowRoutesInloop(controller, controllerId, appId) {
    ...
    var actions = (appId.length > 0 ? Object.keys(sails.apps[appId].controllers[controllerId]) : Object.keys(sails.controllers[controllerId]));

    // Determine base route
    var baseRoute = config.prefix + '/' + (appId.length > 0 ? appId + '/' : '') + controllerId;
    if (config.pluralize) {
    ...
    if (
      (appId === "" && sails.hooks.orm && sails.models && sails.models[controllerId])
      ||
        (appId !== "" && sails.hooks.orm && sails.apps[appId].models && sails.apps[appId].models[controllerId])
      ) {
      ...
      if ( config.shortcuts ) {
        ...
        sails.router.bind(baseRoute + '/find/:id?', _getMiddlewareForShadowRoute(controllerId, 'find', appId), null, routeOpts);
        sails.router.bind(baseRoute + '/create', _getMiddlewareForShadowRoute(controllerId, 'create', appId), null, routeOpts);
        sails.router.bind(baseRoute + '/update/:id?', _getMiddlewareForShadowRoute(controllerId, 'update', appId), null, routeOpts);
        sails.router.bind(baseRoute + '/destroy/:id?', _getMiddlewareForShadowRoute(controllerId, 'destroy', appId), null, routeOpts);
      }

      // Bind "rest" blueprint/shadow routes
      if ( config.rest ) {
        sails.log.silly('Binding RESTful blueprint/shadow routes for controller:',controllerId);
        sails.router.bind('get ' + baseRoute + '/:id?', _getMiddlewareForShadowRoute(controllerId, 'find', appId), null, routeOpts);
        sails.router.bind('post ' + baseRoute, _getMiddlewareForShadowRoute(controllerId, 'create', appId), null, routeOpts);
        sails.router.bind('put ' + baseRoute + '/:id?', _getMiddlewareForShadowRoute(controllerId, 'update', appId), null, routeOpts);
        sails.router.bind('delete ' + baseRoute + '/:id?', _getMiddlewareForShadowRoute(controllerId, 'destroy', appId), null, routeOpts);
      }
	...
	function bindShadowRoutes () {
      ...
      _bindShadowRoutesInloop(controller, controllerId, "");
		});

    _.each(sails.apps, function(app, appId) {
      _.each(app.controllers, function(controller, controllerId) {
        _bindShadowRoutesInloop(controller, controllerId, appId);
      });
    });
   ...
   */
  function _getMiddlewareForShadowRoute (controllerId, blueprintId, appId) {
    //Allow custom actions defined in controller to override blueprint actions.
    return (appId.length > 0 ? sails.apps[appId].middleware.controllers[controllerId][blueprintId] || hook.middleware[blueprintId] : sails.middleware.controllers[controllerId][blueprintId] || hook.middleware[blueprintId]);
  }
	...
	function loadMiddleware (cb) {
        ...
        _.each(sails.apps, function(app, appId) {
          _.each(app.middleware.controllers, function(controller) {
            _.defaults(controller, hook.middleware);
          });
        });
			});
