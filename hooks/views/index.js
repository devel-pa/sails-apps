module.exports = function (sails) {
	...
	return {
		 ...
		 */
		bindView: function ( path, target, verb, options ) {
		...
		detectAndPrepareViews: function (cb) {
			...
			var self = this;
			this.glob(sails.config.paths.views, function (err, detectedViews) {
        ...
        sails.modules.statApps(appsPath, function appsLoaded(err, apps){
          ...
          _.each(apps, function(app, key) {
            if(app.isDirectory) {
              ...
              if(_.has(app, 'views') && app.views.isDirectory) {
                self.glob(appsPath + '/' + key + '/views', function(err, dViews) {
                  sails.apps[key].views = dViews || {};
                  self._createMiddleware(dViews, key);
                });
              }
            }
          });
		...
		glob: function (path, cb) {
			
			// Glob/stat views so we know whether they exist or not
			sails.modules.statViews(path, cb);
		},
		...
		_createMiddleware: function (detectedViews, appId) {
			...
			util.each(detectedViews, function (view, id) {
				...
				if (view === true) {
					...
					this.middleware[id] = this._serveView(id, subViewId, appId);
				}
          ...
          if(appId === "") {
            ...
            for (var subViewId in detectedViews[id]) {
              ...
              this.middleware[id][subViewId] = this._serveView(id, subViewId, appId);
            }
          } else {
            this.middleware[appId] = this.middleware[appId] || {};
            this.middleware[appId][id] = {};
            for (var subViewId in detectedViews[id]) {
              // sails.log.verbose('Building middleware chain for view: ', id, '/', subViewId);
              this.middleware[appId][id][subViewId] = this._serveView(id, subViewId, appId);
            }
          }
				}
		...
		_serveView: function (viewId, subViewId, appId) {
      var self = this;
			// Save for use in closure
			// (handle top-level and subview cases)
			var viewExpression = (appId === "" ? "" : appId + '/') +  viewId + (subViewId ? '/' + subViewId : '');

			return [function rememberViewId(req, res, next) {
        var test = self.middleware;
	...
	function _addResViewMethod(req, res, next) {
		...
		res.view = function (specifiedPath, locals, cb_view) {
			...
			if ( specifiedPath && !util.isObject(specifiedPath)) {
			...
			// If a map of locals is provided as the first argument,
				...
				if (!util.isObject(locals)) {
					relPathToView = locals;
					locals = specifiedPath;
				}

				// var pathAsSecondArg = (locals && !util.isObject(locals) ) ? locals : undefined;
			...
			// Ensure specifiedPath is a string to avoid breaking express
			...
			return res.render(relPathToView, function (err, renderedViewStr) {
				if (err) {
